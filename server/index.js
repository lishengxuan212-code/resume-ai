...
export function createServerApp(env = process.env, { fetchImpl = fetch, services = {} } = {}) {
  ...
  try {
    return createApp({ config: readConfig(env), fetchImpl, services, accessConfig, accessControl, adminControl, staticDir: env.NODE_ENV === 'production' ? path.resolve('dist/client') : null });
  } catch (configError) {
    return createApp({ configError, fetchImpl, services, accessConfig, accessControl, adminControl, staticDir: env.NODE_ENV === 'production' ? path.resolve('dist/client') : null });
  }
}
