...
export function validateOptimizedResume(value, facts, provider, model) {
  ...
  if (!isObject(facts) || !isList(facts.sourceBlocks, 30)) fail();
  ...
  const sections = value.sections.map((section) => {
    ...
    const entries = section.entries.map((entry) => {
      ...
      if (!isList(entry.sourceIds, 30) || !entry.sourceIds.every((id) => typeof id === "string" && ids.has(id))) fail();
...
export function validateOptimizeInput(value) {
  ...
  if (!isList(facts.sourceBlocks, 30)) fail();
  const ids = new Set();
  const sourceBlocks = facts.sourceBlocks.map((block) => {
    if (!isObject(block) || !isText(block.id, 200, true) || !isText(block.text, 12000, true) || ids.has(block.id)) fail();
