...
export function buildPromptSections(stage, input) {
  ...
  const sections = [
    ...
    { id: 'task', stability: 'stage', source: stage, content: stage === 'diagnose'
      ...
      : `生成可投递的中文简历。每条 bullet 必须是“标题—内容”对象：title 用简洁短语说明这段具体做了什么，text 再说明对象、行动、方法和有事实依据的结果；不得用“工作内容”“经历描述”等无信息标题。工作、项目和技能 entry 至少有一条 bullet；教育只有学校、专业、学历和日期而没有课程、奖项或项目事实时，bullets 必须为空数组，不得为了满足结构补造学习内容。技能模块的 heading 必须固定为“技能”，不得使用“专业技能”“专业能力”“技能清单”或“核心技能”；所有技能 entry 的 title、organization、dates 必须为空。技能只使用 bullet 的小标题和正文，并按一行“小标题：正文”展示，例如 title 写“付费与活动玩法”，text 写“具备付费卡点设计、会员体系设计、商品结构规划、抽奖/盲盒/礼包活动设计和活动复盘经验。”；不得把“商业化运营”“用户运营”“产品运营”“工具”“常用工具”“工具使用”“工具能力”“技能清单”“专业技能”“专业能力”用作技能 bullet 标题。工具类内容应按实际用途写成“原型与数据处理”等具体小标题。每条非空 bullet 还必须包含支持事实的 sourceIds 和实际采用的 ruleIds；每条 bullet 的 ruleIds 都必须包含 F01，这是服务端强制校验项。方法论编号只能放在 ruleIds 中，严禁把 F03、E02 等内部代码写入 summary、标题、正文或其他用户可见文字。ruleIds 只能来自本次适用规则：${method.ruleIds.join('、')}。职位、公司和日期忠实保留；不要给已结束的教育经历添加“在读”，不要把 2017.09 改成 2017年9月等新格式；教育 bullet 不要重复日期字段。姓名和联系方式由页面使用 facts.name 与 facts.contact 呈现，不要因它们不在来源片段中就报告缺失。技能必须精简并按可迁移的实际能力表达；企业内部 AI 工具、自研数据看板等不可迁移的内部工具默认省略。只有来源或用户回答明确支持熟练度时才能写“精通”“熟练”，不能从“使用过”推断。个人概述或优势不得堆叠“沟通能力强、逻辑清晰”等空泛标签，必须概括下方真实经历中的任务、领域或成果；证据不足时宁可缩短或留空。不得根据起止日期自行计算任何中文或阿拉伯数字形式的工作年限，例如“2年经验”“约四年经历”“五年相关经验”。不能原样复制整段来源；可以删除与目标岗位无关、重复或事实口径不清的材料，也可以基于已确认事实完成取舍、拆分、重组和专业表达。用户确认的 suggestedRewrite 可作为表达偏好使用，但仍不得扩展成未经确认的新事实。输出 methodologyVersion=${METHODOLOGY_VERSION}。只返回符合指定结构的 JSON。` },
