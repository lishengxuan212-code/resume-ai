...
import { careerContextForPrompt } from './career-knowledge/index.js';
...
export function buildPromptSections(stage, input) {
  ...
  const careerContext = careerContextForPrompt(input);
  const sections = [
    ...
    method,
    { id: 'career-knowledge', stability: 'version', source: `career-knowledge/${careerContext.version}`, content: JSON.stringify(careerContext) },
    { id: 'career-boundary', stability: 'version', source: 'career-knowledge', content: stage === 'diagnose'
      ? `结合 career-knowledge 判断材料属于直接相关、可迁移还是待补证，并把确实影响结果的结论写入 findings。行业背景只能解释岗位环境，不能证明用户适合某个行业。对跨行业材料，优先询问能确认目标任务、个人贡献、作品、课程或真实结果的问题。所有 suggestedRewrite 仍只能使用用户已有来源。`
      : `career-knowledge 只能帮助选择和组织用户已经证明的经历；不得把其中的行业趋势、岗位任务、常见缺口或能力标签写成用户事实。matchMode=transferable 或 gap 时，只能把来源明确的原任务映射为可迁移能力，并保留真实的任务对象和职责边界，不得制造目标行业经验。` },
