...
- Keep all PDF templates as white, black-text A4 single-column resumes. Use a bundled Chinese serif font with Song-style appearance, bold headings, consistent hierarchy and punctuation. `经典`, `简约` and `紧凑` may vary density only; their content area and pagination rules must stay identical.
...
- Keep template selection as an in-flow template-library panel with realistic anonymous resume examples, never abstract wireframes. Do not auto-generate on edits or selection: generation is user initiated, the library stays in place behind a 1:1 PDF modal while it runs, and a preview cache may be reused only when facts, resume content, presentation data and template are unchanged. Do not show an empty PDF-result area before generation. Place the PDF download action beside the document in its modal, never over its content.
...
- When a user skips every diagnosis question and the three AI attempts fail only because their returned resume structure cannot pass validation, continue with a clearly identified fact-only fallback built from reviewed material. Preserve visible failures for authentication, network, timeout and other provider errors.
- For work and internship entries, keep role, organization and dates on one desktop row. Put every non-skill point's bold title on its own line with the body beneath, and use the regular `·` marker. Keep the existing inline `小标题：正文` skill presentation unchanged.
