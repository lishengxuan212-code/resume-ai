...
- Treat the following items as planned V2.0 capabilities, not as currently implemented features.
- Reduce uncertainty about the final deliverable with two homepage entries to the same template library: keep the secondary side tab named `模板库`, and place a `简历模板` secondary button immediately to the left of the primary `上传简历` button inside the upload panel. Both open the same full-page library without changing the approved homepage hero. The enabled `推荐` template uses a locally bundled, fictional-content PDF rendered by the real template, including a square fictional portrait in the upper-right identity area so the preview represents the complete intended layout. Never use a real user's photo in this shared sample. `使用此模板` opens resume upload and then continues to material review; normal homepage upload still defaults to the same template. Keep a `查看模板` entry on material review so users can reopen it. The preview is an appearance example, not a promise that every resume will have the same page count or density.
- Build a versioned offline career-knowledge layer instead of performing live web search. Store source name, publication date, applicable industry, role family, seniority, evidence type and update status. Use report findings for industry context and use role requirements or hiring evidence for resume diagnosis; do not treat broad industry forecasts as proof that a candidate fits a role, and do not append complete reports wholesale to every model prompt.
- Industry guidance must be expressed as evidence-based candidate directions, relevant experience, transferable evidence, missing evidence and suggested next actions. Do not state that a person is definitively suitable or unsuitable for an industry, do not generate hiring probabilities or opaque numeric fit scores, and make stale or weak knowledge visible rather than presenting it as current fact.
- When the reviewed resume has little direct evidence for the selected target, enter a cross-industry transition path instead of returning a normal target-role rewrite. Map each usable source experience through `source task -> demonstrated transferable capability -> target task -> evidence strength -> conservative rewrite`, keep unsupported target-domain claims out of the resume, and separately identify gaps that require user clarification, portfolio work, coursework or real experience.
- Distinguish direct evidence, transferable evidence and missing evidence in diagnosis. Let the user supply a target job description or answer focused transition questions, while still allowing a conservative no-answer path. Cross-industry optimization must improve framing and selection of real experience; it must never manufacture target-industry experience, metrics, terminology mastery or responsibility boundaries.
- Present diagnosis as annotations attached to the reviewed resume. Number annotations in the same top-to-bottom order as the rendered resume and place each number beside the exact matched phrase or line. A shared source ID is never sufficient for visual placement; if no reliable text match exists, keep the item in the whole-resume annotation group. Keep the issue, revision direction and any linked fact question together in the annotation rail.
- Do not show a standalone `岗位要求覆盖` module on the diagnosis page. If a target job description affects diagnosis, express only concrete, source-grounded issues through the normal resume annotations; do not add a coverage scorecard or claim majority-market fit.
- Keep the diagnosis rail heading fully visible. Clicking a resume annotation number scrolls only the right-hand comment list and aligns the matching comment at the top; it must not center the card or move the whole page. `查看原文位置` remains the action that scrolls the resume source.
