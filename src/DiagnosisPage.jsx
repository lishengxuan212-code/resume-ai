...
export function DiagnosisPage({ diagnosis, busy, statusText, error, onBack, onOptimize }) {
  ...
  return <div className="review-page diagnosis-page">
    ...
    <main className="diagnosis-workspace">
      ...
      {!!diagnosis.questions.length && <section className="diagnosis-card"><div className="review-section-heading"><h2>补充问题</h2><span>仅展示确实影响表达的事实 · 均可跳过</span></div>
        {diagnosis.questions.map((item, index) => <div className="diagnosis-question" key={item.id}><span><Question size={17} />问题 {index + 1}</span><strong>{item.question}</strong><small>{item.reason}</small><div className="diagnosis-suggestion"><span><Lightbulb size={16} />不知道怎么写？可以先采用这条保守表达</span><p>{item.suggestedRewrite}</p><button type="button" disabled={busy} onClick={() => setAnswers({ ...answers, [item.id]: item.suggestedRewrite })}>采用建议表达</button></div><label><span className="visually-hidden">问题 {index + 1} 的回答</span><textarea value={answers[item.id]} disabled={busy} maxLength={3000} placeholder="补充真实情况，或采用上面的建议后继续修改。" onChange={event => setAnswers({ ...answers, [item.id]: event.target.value })} /></label></div>)}
      </section>}
      <section className="diagnosis-actions" aria-busy={busy}><p className={error ? 'error' : 'processing-status'}>{error || statusText}</p><div><button className="button secondary" disabled={busy} onClick={() => onOptimize([], true)}>跳过问题，按现有材料优化</button><button className="button primary" disabled={busy} onClick={() => onOptimize(answered, false)}>{busy ? '正在优化简历…' : '使用补充事实优化'}<ArrowUpRight size={18} /></button></div></section>
    </main>
  </div>;
}
