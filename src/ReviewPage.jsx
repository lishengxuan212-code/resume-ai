      ...
      <main className="review-main">
        <form onSubmit={onDiagnose} aria-busy={busy}>
          <fieldset className="editor-fields" disabled={busy}>
            ...
            <section className="review-section source-section" id="review-sources"><div className="review-section-heading"><h2>来源原文</h2><span>留在这里，方便对照</span></div><p className="section-caption">原文完整保留，无需逐项勾选。你修改或补充的经历会自动记录。</p>
              ...
              <button type="button" className="add-entry" disabled={facts.sourceBlocks.length >= 30} onClick={() => { let number = 1; while (facts.sourceBlocks.some(block => block.id === `review-b${number}`)) number++; onFacts({ ...facts, sourceBlocks: [...facts.sourceBlocks, { id: `review-b${number}`, text: '', page: null }] }); }}><Plus size={16} />补充来源原文</button>
            </section>
          </fieldset>
          ...
          <div className="review-submit-area">
            ...
            <div className="service-status">{configLoading ? '正在检查 AI 服务…' : configError || (config?.configured ? `使用 ${providerNames[config.provider] || config.provider} 优化 · ${config.model}` : '当前 AI 服务尚未配置')}{!config?.configured && <button type="button" className="text-button" disabled={busy || configLoading} onClick={onReadConfig}>重新检查服务</button>}</div>
