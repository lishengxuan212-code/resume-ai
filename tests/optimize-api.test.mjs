...
const facts = { name: "张三", contact: "", education: [], experiences: [], skills: [], warnings: [], sourceBlocks: [{ id: "p1-b1", text: "负责用户访谈", page: 1 }] };
...
test("optimize normalizes injected results with server provenance and strips extra fields", async () => {
  ...
  assert.deepEqual(JSON.parse(result.text), { resume: { ...resume, provider: "openai", model: "server-model" } });
  assert.deepEqual(received, { facts, targetRole: "产品助理" });
...
test("injected output still rejects nonexistent sources", async () => {
  ...
  invalid.sections[0].entries[0].sourceIds = ["unknown"];
