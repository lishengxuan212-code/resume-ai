...
test('export downloads the PDF blob then releases its URL and removes the anchor', async () => {
  ...
  const client = create({
    fetch: async (url, init) => { assert.equal(url, '/api/export'); assert.deepEqual(JSON.parse(init.body), { facts: { name: '张三' }, resume: { summary: '结果' } }); return new Response('%PDF-1.7', { headers: { 'Content-Type': 'application/pdf' } }); },
