...
test('diagnosis can be skipped while internal rule codes stay out of the user interface',async t=>{
  ...
  await page.getByRole('button',{name:'生成预览 PDF',exact:true}).click();
  ...
  await page.locator('.template-card').filter({hasText:'经典'}).click();
  await page.getByRole('button',{name:'生成预览 PDF',exact:true}).waitFor();
  assert.equal(exportCalls,1);
  await page.locator('.template-card').filter({hasText:'推荐'}).click();
  await page.getByRole('button',{name:'显示已有预览',exact:true}).click();
  assert.equal(exportCalls,1);
